...
class TypesController extends Controller
{
    ...
    public function index($slug)
    {
        ...
        //查询当前分类
        $type=DB::table('categories')->where('slug',$slug)->first();
        //将path路径处理成数组
        $arr=explode(',', $type->category_path);
        $size=count($arr)-1;
        switch ($size) {
            case '1'://一级分类下的产品
                $erClass=DB::table('categories')->where("category_path","like","%,$type->id,%")->get();
                $newArr=array();
            ...
            case '2'://二级分类时直接查询当前分类下所有产品
                $products=DB::table('products')->where('pro_category',$type->id)->get();
