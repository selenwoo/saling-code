      ...
      <div class="col-xl-9 col-lg-9 col-md-9 productRight">
        <div class="portfolio">
          <ul>
            ...
            <li class="mybigclass"><span class="product_titlebg2"><i class="fa fa-angle-double-right"></i>{{ ($locale=='en') ? $category->categorynameen : $category->category_name }}</span><a href="{{ url('products-list/'.$category->slug) }}"></a></li>
            <li>
              <ul>
                @foreach ($category->zi as $twoType)
                <li>
                  <div class="product_titlebg3"><span class="">{{ ($locale=='en') ? $twoType->categorynameen : $twoType->category_name}}</span><a href=""></a></div>
                  <div class="portfolio">
                    <ul class="row">
                      @foreach ($twoType->myproducts as $product)
                      <li class="col-lg-3 col-md-3 col-sm-3 col-xs-6 myheight">
                        <div class="pro-list"> <a href="{{ url('products/'.$product->slug) }}">
                          <div class="imgDiv"><img class="img-fluid" src="{{ $product->pro_listimg }}" alt="{{ $product->pro_name }}" />
                            <div class="rotate"><i> </i></div>
                          </div>
                          <div class="modelDiv">
                            <div>{{ ($locale=='en') ? $product->pronameen : $product->pro_name }}</div>
                          </div>
                          </a> </div>
                      </li>
                      @endforeach 
